...
export async function GET(request) {
    ...
    try {
        ...
        // 取得營收資料（CRM：已付款的客戶）
        const revenuePages = CRM_DB_ID ? await queryDatabase(CRM_DB_ID, {
            and: [
        ...
        const monthlyData = months.map(({ key }) => {
            const monthRevenue = revenuePages
                ...
                .reduce((sum, p) => sum + (getPropValue(p, '課程金額') || 0), 0);
        ...
        console.error('Profit dashboard API error:', err);
        return NextResponse.json({ error: '取得營運數據失敗' }, { status: 500 });
    }
}
